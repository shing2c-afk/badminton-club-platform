...
// 💡 [수정됨] 프론트엔드에서 억지로 묻던(이중 모달 유발) 코드를 제거하고,
// 의사(emit)만 서버로 보내면, 똑똑한 서버가 socket.js를 통해 정확한 모달을 1번만 띄웁니다!
    ...
    if (rawUser) {
        try {
            ...
            try {
                ...
                if (parsed) userData = parsed; // 객체 형태의 유저 정보 전체 전송
            } catch (e) {
                userData = { name: rawUser };
            }

            // 서버의 /api/logout 라우터로 유저 정보를 전달하여
            // "이 유저가 속한 방에 혼자 남았다면 폭파하라"는 서버 로직이 정상 작동하도록 유도합니다.
